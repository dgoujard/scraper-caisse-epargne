...
const getAmountFromText = text => parseFloat(text.replace(',', '.').replace('&nbsp;', '').replace(' ', '').replace('€', '').trim())
const waitLoader = () => {
  console.log('wait until loader is not visible')
  return driver.wait(until.elementIsNotVisible(driver.findElement(By.xpath(`//div[@class="loaderServeur"]`))))
}
...
const signIn = () => {
  return new Promise((resolve, reject) => {
    ...
    console.log('show accessibility keyboard')
    ...
    passwordButton.click()
      .then(() => {
        ...
        console.log('click "Mes comptes" menu link')
...
const chooseAccount = (account) => {
  return new Promise((resolve, reject) => {
    ...
    console.log('click "Ma Synthèse" menu link')
    ...
    syntheseLink.click()
      .then(() => {
        ...
        // click account link
        console.log(`click account link ${account.bankId}`)
